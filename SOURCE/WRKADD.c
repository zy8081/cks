    ...
    while(1)
    {
        ...
        if(mouse_press(442,420,582,480)==1)
        {
            if(strcmp(*pname0,"\0")!=0&&*pplacenum!=0)
            {
                ...
                sprintf(filen,"WORKS\\%s\\%s",user,*pname0);

                //debug_file_printf(filen);
                sprintf(filen_dat,"WORKS\\%s\\%s\\%s.dat",user,*pname0,*pname0);
                sprintf(filen_bas,"WORKS\\%s\\%s\\%s.bas",user,*pname0,*pname0);
                sprintf(filen_his,"WORKS\\%s\\%s\\%s.his",user,*pname0,*pname0);
                if(access(filen,0)==0)//
                {
                    put_hz24(260,230,"命名已重复！！",0,"HZK\\HZK24",0);  
                    for(i=0;i<=1000;i++)
                    {
                        delay(1);
                        mouse_renew(&MouseX,&MouseY,&press);
                    }
                    put_hz24(260,230,"命名已重复！！",27469,"HZK\\HZK24",0);
                    continue;
                }
                else
                {
            ...
            else
            {
                ...
                {
                    put_hz24(260,230,"未输入项目名称！！",0,"HZK\\HZK24",0);
                }

                if(*pplacenum==0)
                {
                    put_hz24(260,350,"未选择地点！！",0,"HZK\\HZK24",0);
                }
                for(i=0;i<=1000;i++)
                {
                    delay(1);
                ...
                put_hz24(260,230,"未输入项目名称！！",27469,"HZK\\HZK24",0);
                put_hz24(260,350,"未选择地点！！",27469,"HZK\\HZK24",0); 
            }
        }
    }
}
...
void placeprt(int placenum)
{
    ...
    {
        case 0:
            put_hz24(420,290,"点击按钮选择地点",65535,"HZK\\HZK24",0);
            break;
        case OlymMon:
            put_hz24(420,290,"奥林匹斯山",65535,"HZK\\HZK24",0);
            break;
        case MariVal:
            put_hz24(420,290,"水手大峡谷",65535,"HZK\\HZK24",0);
            break;
        case ArabTer:
            put_hz24(420,290,"阿拉伯高地",65535,"HZK\\HZK24",0);
            break;
        case UtopPla:
            put_hz24(420,290,"乌托邦平原",65535,"HZK\\HZK24",0);
            break;
        case ElysPla:
            put_hz24(420,290,"埃律西昂平原",65535,"HZK\\HZK24",0);
            break;
    }
}
...
void datacreate(char *filen)
{
    ...
    {
        CloseSVGA();
        printf("can not open file:%s",pathbuild);
        getch();
        exit(0);
    }
